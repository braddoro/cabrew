...
$primaryKey = 'corporateDonationID';
$conn = new Connect();
$db = $conn->conn();
if(!$db->isConnected()){
...
case 'fetch':
	$where = '1=1';
	if(isset($_REQUEST['coporationID_fk'])){
		$where .= " and coporationID_fk = " . intval($_REQUEST['coporationID_fk']);
	}
	if(isset($_REQUEST['eventTypeID_fk'])){
		$where .= " and eventTypeID_fk = " . intval($_REQUEST['eventTypeID_fk']);
	}
	break;
case 'add':

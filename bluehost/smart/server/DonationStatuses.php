...
$table = 'dontationStatuses';
$primaryKey = 'donationStatusID';
$conn = new Connect();
$db = $conn->conn();
// $db->debug = true;
if(!$db->isConnected()){
...
case 'remove':
	$where = $primaryKey . '=' . $pkval;
	$sql = "delete from {$table} where {$where} order by donationStatus;";
	$db->execute($sql);
	break;
default:
	break;
...
$sql = "select * from {$table} where {$where} order by donationStatus;";
